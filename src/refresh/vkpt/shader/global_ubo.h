...
#define UBO_CVAR_LIST \
	...
	UBO_CVAR_DO(pt_blood_normal_speed, 1.0) /* how fast the ripple animates */ 	UBO_CVAR_DO(pt_fog_step, 8.0) /* multiplies the fog march's step length. 1 = the original step; 8 is Matt's tuned default. THE COST KNOB - see getStep in fog_medium.glsl, and read it before touching cl_fog_scale */ \
	...
	UBO_CVAR_DO(pt_glass_secondary_flat, 1) /* THIN GLASS SEEN IN A REFLECTION OR THROUGH ANOTHER PANE - how the second pane picks between reflecting and letting the view through (reflect_refract.rgen). Only the FIRST pane splits the two fields; every later pane follows one path, weighted by F or 1-F. 0 = old rule: refract when F < 0.1, with F from the NORMAL-MAPPED normal. At that threshold the pixel jumps between 0.9 x the view through and 0.1 x a different reflected image, and because the normal map is resampled under the sub-pixel jitter every frame, pixels near it flip frame to frame - the sparkle in reflections of glass (pipes behind a pane seen in another pane) that A-SVGF blurs away and DLSS-RR cannot. 1 = decide on the FLAT geometric normal, so the choice is stable under jitter and draws one clean edge across the pane, and at F = 0.5, where F and 1-F are equal, so the brightness is continuous across that edge. The weights still use the normal-mapped F. APPENDED AT THE END. */ \
	UBO_CVAR_DO(pt_pom, 1) /* parallax occlusion mapping for materials with a height map (texture_height / <name>_height.tga), ported from RTX Remix. 0 = off, 1 = quadtree (Remix's default rtx.displacement.mode, QuadtreePOM: an exact per-texel trace over max-filtered height mips), 2 = raymarch (Remix RaymarchPOM). Applied at primary and reflection/refraction hits. */ \
	UBO_CVAR_DO(pt_pom_scale, 1) /* global multiplier on every material's displace_in and displace_out, [0..inf) */ \
	UBO_CVAR_DO(pt_pom_max_steps, 64) /* Remix rtx.displacement.maxIterations: the iteration cap for pt_pom 1, and the layer count at grazing angles for pt_pom 2 (a head-on view uses a quarter). */
