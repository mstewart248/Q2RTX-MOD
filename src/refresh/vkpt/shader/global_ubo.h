...
#define UBO_CVAR_LIST \
	...
	UBO_CVAR_DO(pt_pom, 1) /* parallax occlusion mapping for materials with a height map (texture_height / <name>_height.tga), 0 or 1. Applied at primary and reflection/refraction hits; bounce hits use a low mip of the base texture only and are left alone. */ \
	UBO_CVAR_DO(pt_pom_scale, 1) /* global multiplier on every material's displace_in and displace_out, [0..inf) */ \
	UBO_CVAR_DO(pt_pom_max_steps, 32) /* march steps at grazing angles; a head-on view uses a quarter of this. More steps = fewer stair-step layers on deep displacement. */
