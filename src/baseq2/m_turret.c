...
/* Rogue's non-truncating vectoangles. This tree's vectoangles() rounds yaw and
   pitch to whole degrees with an (int) cast, which is too coarse for the
   turret's aim tracking - it hunts around the target instead of settling. */
...
turret_checkattack(edict_t *self)
{
	...
	{
		self->monsterinfo.attack_state = AS_MISSILE;
		self->monsterinfo.attack_finished = level.framenum + nexttime * BASE_FRAMERATE;
		return true;
	}
