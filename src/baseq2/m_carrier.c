/* =======================================================================
 *
 * The carrier. A flying boss that spawns flyers out of a hatch in its belly,
 * and the first monster here that cares which side of itself a player is on -
 * it will not rail or machine-gun someone behind or underneath it, only lob
 * rockets back at them.
 *
 * Ported from rogue with the same mechanical pass as m_turret.c: the yamagi
 * null guards are gone, and every float level.time site is now this tree's
 * integer level.framenum. Two things rogue has that this tree does not are
 * called out where they were dropped - trigger_bad_area, and the file-global
 * orig_yaw_speed (which would not have survived a savegame anyway).
 *
 * =======================================================================
 */
...
#define CARRIER_ROCKET_TIME (2 * BASE_FRAMERATE)    /* between rocket shots */
#define CARRIER_YAW_SPEED 15
...
CarrierCoopCheck(edict_t *self)
{
	...
	self->wait = level.framenum + CARRIER_ROCKET_TIME;
...
CarrierGrenade(edict_t *self)
{
	...
	mytime = (level.framenum - self->timestamp) / 4;
...
CarrierSpawn(edict_t *self)
{
	...
	mytime = (level.framenum + 1 - self->timestamp) / 5;
...
carrier_spawn_check(edict_t *self)
{
	...
	if (level.framenum > (self->timestamp + 11))  /* 0.5 seconds per flyer.  this gets three */
	{
		self->monsterinfo.aiflags &= ~AI_MANUAL_STEERING;
...
CarrierRail(edict_t *self)
{
	...
	self->monsterinfo.attack_finished = level.framenum + RAIL_FIRE_TIME;
}
...
carrier_attack(edict_t *self)
{
	...
	/* rogue also has a branch here for standing inside a trigger_bad_area.
	   That entity does not exist in this tree, so the condition can never be
	   true and the branch is gone rather than left unreachable. */
...
carrier_reattack_gren(edict_t *self)
{
	...
	if (infront(self, self->enemy))
	{
		if (self->timestamp + 13 > level.framenum)  /* four grenades */
		{
			self->monsterinfo.currentmove = &carrier_move_attack_gren;
			return;
		}
