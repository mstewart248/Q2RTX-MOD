...
/*
 * Small rogue helpers the stalker needs. They live here rather than in
 * g_monster.c because nothing else in this tree uses them yet; move them out
 * if the carrier or the widow ever want them too.
 */
static bool has_valid_enemy(edict_t *self)
{
    return self->enemy && self->enemy->inuse && self->enemy->health >= 1;
}

/* This tree's vectoangles() truncates yaw and pitch to whole degrees with an
   (int) cast. That is fine for aiming, but the stalker feeds these angles
   straight into the ballistic jump solve below, so keep the precise form. */
...
SP_monster_stalker(edict_t *self)
{
	...
	/* SPAWNFLAG_STALKER_ONROOF - flip the model over and invert its gravity so
